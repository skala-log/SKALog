...
### F. 홈 "조잡함" 개선 — 2열 재배치안 확정, D1/M1 에 반영 (완료)
...
**안 B(2열 재배치)** 로 시안(`D1-b`/`M1-b`) 제작 → **사용자 확정 → 기존 D1/M1 을 대체함.**

- **D1** : 상단 얇은 스트립 / 좌 = 오늘 강의 · 공지 · 내 최근 기록 / 우 280 = 바로가기 2×3 · 이번 주 · 식단
- **M1** : 히어로 440 → **79px**(인사+날짜+진행만), 오늘 강의는 흰 카드로 분리
- 콘텐츠 축소 없음(사용자 지시). 여백만 조임

> ⚠️ 사용자가 Pencil 에서 직접 D1-b 를 재배치했다(이번 주를 우측으로, 최근 기록을 좌측으로,
> 식단을 `칩 + 점심/저녁 탭 + 상세` 구조로 교체, `ClassMode`·`NoticeScope` 컴포넌트 신규).
> 그 구조를 그대로 살렸다. **Column 높이가 942px 라 900 프레임에서 아래가 잘린다** —
> 모바일 프레임과 같은 "폴드 아래는 잘린다" 규칙을 따른 것이며, 실제 화면에선 약 106px 스크롤된다.
> 한 화면에 꼭 넣어야 하면 좌측 컬럼(845px)에서 106px 을 덜어내야 한다.

### G. 프레임 전면 정리 (완료)

- **화면 번호 재부여** — 월간/쇼케이스가 뒤에 붙어 순서가 꼬여 있던 것을 정리
- **캔버스 4행 정렬** — y=0 보드류 / y=1660 모바일 / y=2592 상태 / y=3600 데스크톱 / y=4660 관리자
- **`01 · Components` 판 재편성** — 섹션 라벨 7개(`기본 요소 / 레이아웃 / 컴포저 / 피드백 / 셸 / 홈 블록 / Hover 상태`)로
  그룹핑하고 낡은 라벨 9개 제거. `Sidebar` 가 `height: fill_container` 라 갤러리에서 1px 로 붕괴하던 것을 720 고정으로 수정
- **`02 · 흐름판` 화면 번호 갱신** — 재번호로 어긋난 참조 8곳 수정
## `.pen` 최상위 프레임 목록 (2026-08-03 기준 · 30개)

```
[보드]      y=0
u58CS9  00 · Tokens (x0)     Ta8Oo  01 · Components (x980)   e6PnK1  02 · 흐름판 (x1960)

[모바일]    y=1660 · x 는 435 간격
QSiEe  M1 · 홈              qNqFY  M2 · 일정표            rknlZ  M3 · 일정표 (월간)
y30Qw1 M4 · 일정 상세        G2TPg  M5 · 내 기록           LlNDU  M6 · 쇼케이스
EEDNh  M7 · 로그인           fey9g  M8 · 반 선택

[상태]      y=2592 · x 는 435 간격
bSwvB  S1 · 홈 / 로딩        bMd4o  S2 · 홈 / 강의 없음     p11orv S3 · 홈 / 카드 에러
Fv0J1  S4 · 아코디언 닫힘     k5uog  S5 · 컴포저 펼침        HULce  S6 · 저장 직후
W50odo S7 · 자료 없음        jecJH  S8 · 내 기록 빈 상태

[데스크톱]  y=3600 · x 는 1520 간격
O8LLed D1 · 홈              QNwiq  D2 · 일정표 (월간)      UJ18c  D3 · 일정 상세
qDmVh  D4 · 내 기록          lkgZA  D5 · 쇼케이스

[관리자]    y=4660 · x 는 1520 간격
XOFB4  A1 · 자료 승인함      TMhSV  A2 · 일정 관리
A3uj2e A2-a · 행 인라인 편집  k0JTK  A2-b · 삭제 경고
```

> ★ `D1`/`M1` 의 노드 id 가 바뀌었다 (`mV6Fp`→`O8LLed`, `ma3xy`→`QSiEe`).
> 예전 문서·메모에 남아 있는 옛 id 를 그대로 쓰면 "노드 없음" 이 난다.
...
| `/timeline?view=month` | 일정표(월간) | `rknlZ`(M3) / `QNwiq`(D2) |
| `/timeline/:id` | 일정 상세 | `y30Qw1`(M4) / `UJ18c`(D3) |
| `/records` | 내 기록 | `G2TPg`(M5) / `qDmVh`(D4) |
| `/showcase` | 쇼케이스 | `LlNDU`(M6) / `lkgZA`(D5) |
| `/login` | 로그인 | `EEDNh`(M7) |
| `/onboarding/class` | 반 선택 | `fey9g`(M8) |
...
### 1. ★ frontend `Home.tsx` 를 확정된 D1/M1 구조로 맞추기

`.pen` 은 2열 재배치안으로 확정·반영됐는데 **frontend 는 아직 이전 4층 세로 구조**다. 대조하면 전부 drift 로 잡힌다.

- 데스크톱(`lg:`) — 좌측 컬럼: 오늘 강의 · 공지 · 내 최근 기록 / 우측 280px: 바로가기 2×3 · 이번 주 · 식단
- 모바일 — 히어로는 인사+날짜+진행바만, 오늘 강의는 흰 카드로 분리
- 식단이 `5열 스트립` → **`요일 칩 + 점심/저녁 탭 + 상세 패널`** 로 바뀌었다 (`MealStrip` 재작성 필요)
- `ClassMode`(`Ja22a`) · `NoticeScope`(`g6R1Gq`) 컴포넌트가 새로 생겼다 — 공지에 `우리반`/`전체` 스코프 배지가 붙는다. 프론트엔 아직 없음
